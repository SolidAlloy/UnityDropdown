namespace UnityDropdown.Editor
{
    ...
    public static class MultiKeyQuickSort
    {
        ...
        private static void SortInPlace<T>(IList<DropdownItem<T>> input, int a, int n, int depth)
        {
            if (n < 10)
            {
        ...
        private static void InsertionSort<T>(IList<DropdownItem<T>> x, int a, int n, int depth)
        {
        ...
        private static int MedianOf3<T>(IList<DropdownItem<T>> x, int a, int b, int c, int depth)
        {
        ...
        private static void VecSwap<T>(IList<DropdownItem<T>> x, int a, int b, long n)
        {
            while (n-- > 0)
            {
                (x[a], x[b]) = (x[b], x[a]);
